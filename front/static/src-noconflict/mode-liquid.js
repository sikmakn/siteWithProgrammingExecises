ace.define("ace/mode/doc_comment_highlight_rules", ["require", "exports", "module", "ace/lib/oop", "ace/mode/text_highlight_rules"], function (e, t, n) {
...
}), ace.define("ace/mode/behaviour/liquid", ["require", "exports", "module", "ace/lib/oop", "ace/mode/behaviour", "ace/mode/behaviour/xml", "ace/token_iterator", "ace/lib/lang"], function (e, t, n) {
    "use strict";
    ...
    var r = e("../../lib/oop"), i = e("../behaviour").Behaviour, s = e("./xml").XmlBehaviour,
        o = e("../../token_iterator").TokenIterator, u = e("../../lib/lang"), f = function () {
            s.call(this), this.add("autoBraceTagClosing", "insertion", function (e, t, n, r, i) {
                if (i == "}") {
                    var s = n.getSelectionRange().start, u = new o(r, s.row, s.column),
                        f = u.getCurrentToken() || u.stepBackward();
                    if (!f || !(f.value.trim() === "%" || a(f, "tag-name") || a(f, "tag-whitespace") || a(f, "attribute-name") || a(f, "attribute-equals") || a(f, "attribute-value"))) return;
                    if (a(f, "reference.attribute-value")) return;
                    if (a(f, "attribute-value")) {
                        var l = u.getCurrentTokenColumn() + f.value.length;
                        if (s.column < l) return;
                        if (s.column == l) {
                            var c = u.stepForward();
                            if (c && a(c, "attribute-value")) return;
                            u.stepBackward()
                        }
                    }
                    if (/{%\s*%/.test(r.getLine(s.row))) return;
                    if (/^\s*}/.test(r.getLine(s.row).slice(s.column))) return;
                    while (!f.type != "keyword.block") {
                        f = u.stepBackward();
                        if (f.value == "{%") {
                            for (; ;) {
                                f = u.stepForward();
                                if (f.type === "keyword.block") break;
                                if (f.value.trim() == "%") {
                                    f = null;
                                    break
                                }
                    ...
                    if (!f) return;
                    var h = u.getCurrentTokenRow(), p = u.getCurrentTokenColumn();
                    if (a(u.stepBackward(), "end-tag-open")) return;
                    var d = f.value;
                    h == s.row && (d = d.substring(0, s.column - p));
                    if (this.voidElements.hasOwnProperty(d.toLowerCase())) return;
                    return {text: "}{% end" + d + " %}", selection: [1, 1]}
                }
            })
        };
    r.inherits(f, i), t.LiquidBehaviour = f
}), ace.define("ace/mode/liquid_highlight_rules", ["require", "exports", "module", "ace/lib/oop", "ace/mode/text_highlight_rules", "ace/mode/html_highlight_rules"], function (e, t, n) {
    "use strict";
    var r = e("../lib/oop"), i = e("./text_highlight_rules").TextHighlightRules,
        s = e("./html_highlight_rules").HtmlHighlightRules, o = function () {
            s.call(this);
            var e = "date|capitalize|downcase|upcase|first|last|join|sort|map|size|escape|escape_once|strip_html|strip_newlines|newline_to_br|replace|replace_first|truncate|truncatewords|prepend|append|minus|plus|times|divided_by|split",
                t = "capture|endcapture|case|endcase|when|comment|endcomment|cycle|for|endfor|in|reversed|if|endif|else|elsif|include|endinclude|unless|endunless|style|text|image|widget|plugin|marker|endmarker|tablerow|endtablerow",
                n = "for|if|case|capture|unless|tablerow|marker|comment", r = "forloop|tablerowloop", i = "assign",
            ...
            for (var u in this.$rules) this.$rules[u].unshift({
                token: "variable",
                regex: "{%",
                push: "liquid-start"
            }, {token: "variable", regex: "{{", push: "liquid-start"});
            this.addRules({
                "liquid-start": [{token: "variable", regex: "}}", next: "pop"}, {
                    token: "variable",
                    regex: "%}",
                    next: "pop"
                }, {token: "string", regex: '["](?:(?:\\\\.)|(?:[^"\\\\]))*?["]'}, {
    ...
    r.inherits(o, i), t.LiquidHighlightRules = o
}), ace.define("ace/mode/liquid", ["require", "exports", "module", "ace/lib/oop", "ace/mode/text", "ace/mode/html", "ace/mode/html_completions", "ace/mode/behaviour/liquid", "ace/mode/liquid_highlight_rules", "ace/mode/matching_brace_outdent"], function (e, t, n) {
    var r = e("../lib/oop"), i = e("./text").Mode, s = e("./html").Mode, o = e("./html_completions").HtmlCompletions,
        u = e("./behaviour/liquid").LiquidBehaviour, a = e("./liquid_highlight_rules").LiquidHighlightRules,
        f = e("./matching_brace_outdent").MatchingBraceOutdent, l = function () {
            this.HighlightRules = a, this.$outdent = new f, this.$behaviour = new u, this.$completer = new o
        };
    r.inherits(l, i), function () {
        this.blockComment = {
            start: "<!--",
            end: "-->"
        }, this.voidElements = (new s).voidElements, this.getNextLineIndent = function (e, t, n) {
            var r = this.$getIndent(t), i = this.getTokenizer().getLineTokens(t, e), s = i.tokens, o = i.state;
            if (s.length && s[s.length - 1].type == "comment") return r;
            if (e == "start") {
                var u = t.match(/^.*[\{\(\[]\s*$/);
            ...
            this.$outdent.autoOutdent(t, n)
        }, this.$id = "ace/mode/liquid", this.snippetFileId = "ace/snippets/liquid"
    }.call(l.prototype), t.Mode = l
});
(function () {
