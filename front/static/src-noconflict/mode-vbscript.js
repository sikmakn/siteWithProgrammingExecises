ace.define("ace/mode/vbscript_highlight_rules", ["require", "exports", "module", "ace/lib/oop", "ace/mode/text_highlight_rules"], function (e, t, n) {
    ...
    var r = e("../lib/oop"), i = e("./text_highlight_rules").TextHighlightRules, s = function () {
        ...
        this.$rules = {
            start: [{token: ["meta.ending-space"], regex: "$"}, {
                token: [null],
                regex: "^(?=\\t)",
                next: "state_3"
            }, {
            ...
            state_4: [{
                ...
                regex: "(  )(  )?"
            }, {token: "meta.leading-space", regex: "(?=[^ ])", next: "start"}, {defaultToken: "meta.leading-space"}],
            comment: [{
    ...
    r.inherits(u, i), function () {
        this.indentKeywords = {
            ...
            var r = e.getLine(n), i = /(\w+)\b/i.exec(r), s = i && i[1].toLowerCase();
            if (s && this.indentKeywords.hasOwnProperty(s)) return this.vbsBlock(e, n, i.index + 1)
        }, this.getFoldWidget = function (e, t, n) {
            var r = e.getLine(n), i = /^\s*(\w+)\b/i.exec(r), s = i && i[1].toLowerCase();
            ...
            var f = a.value.toLowerCase(), l = a.value.toLowerCase(), c = [l], h = this.indentKeywords[l];
            if (!h) return;
            var p = i.getCurrentTokenRange(), d = "";
            switch (l) {
                case"property":
                case"sub":
                case"function":
                ...
                case"while":
                case"with":
                    var v = e.getLine(t), m = /^\s*(If)\s+(.)*\s+Then\s+(\S)+/i.test(v);
                    if (m) return;
                ...
                case"end":
                    ...
                    if (a) {
                        ...
                        if (l in u) {
                            f = l;
                            var w = i.getCurrentTokenPosition(), E = w.column + l.length;
                            p = new s(b.row, b.column, w.row, E)
                        }
                    }
                    i.step = i.stepBackward, i.step(), i.step()
            }
            var S = h === -1 ? e.getLine(t - 1).length : e.getLine(t).length, x = t, T = [];
            ...
            while (a = i.step()) {
                ...
                switch (l) {
                    ...
                    case"while":
                    case"with":
                        var v = e.getLine(i.getCurrentTokenRow()), m = /^\s*(If)\s+(.)*\s+Then\s+(\S)+/i.test(v);
                        m && (k = 0, C = !0);
                        var g = new RegExp("^\\s*" + d + l, "i");
                        g.test(v) || (k = 0, C = !0);
                        break;
                ...
                if (k > 0) c.unshift(l); else if (k <= 0 && C === !1) {
                    c.shift();
                    if (!c.length) {
                        switch (l) {
                            case"end":
    ...
    "use strict";
    var r = e("../lib/oop"), i = e("./text").Mode, s = e("./vbscript_highlight_rules").VBScriptHighlightRules,
        o = e("./folding/vbscript").FoldMode, u = e("../range").Range, a = function () {
    ...
    r.inherits(a, i), function () {
        ...
        this.getNextLineIndent = function (e, n, r) {
            var i = this.$getIndent(n), s = 0, o = this.getTokenizer().getLineTokens(n, e), u = o.tokens;
